...
PostShader::PostShader(Device* device, SwapChain* swapchain, VkRenderPass* renderPass, std::string vertPath, std::string fragPath)
	: ShaderProgram(device, swapchain, renderPass)
{
	shaderFiles.push_back(vertPath);
	shaderFiles.push_back(fragPath);
    CreateShaderProgram();
}

void PostShader::CreateShaderProgram() {
    VkShaderModule vertShaderModule = ShaderModule::Create(shaderFiles[0], device->GetVkDevice());
    VkShaderModule fragShaderModule = ShaderModule::Create(shaderFiles[1], device->GetVkDevice());
...
void PostShader::BindShaderProgram(VkCommandBuffer& commandBuffer) {
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 3, 1, &Descriptor::uiParamDescriptorSet, 0, nullptr);
}